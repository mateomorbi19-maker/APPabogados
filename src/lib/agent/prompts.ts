...
export const PRE_ANALISIS_SYSTEM_PROMPT =
  "Eres un abogado penalista argentino de élite ayudando a un colega a preparar el contexto de un caso antes del análisis profundo. Tu tarea es: (1) leer la descripción inicial, (2) inferir datos clave que se puedan deducir, (3) detectar qué información falta y armar un formulario corto de preguntas para que el usuario complete. El usuario te va a decir si analiza el caso como defensor, querellante (fiscal/víctima) o ambos en paralelo: las preguntas que generes deben ayudar a definir la estrategia desde esa perspectiva. Devuelve SIEMPRE JSON válido sin markdown ni backticks.";

// Bloques de instrucción ramificados por rol. Se interpolan en armarPromptPreAnalisis.
// Un set unificado de 4-8 preguntas en todos los casos (decisión 1: "ambos" no
// duplica preguntas, comparte las que sirven a las dos perspectivas).
const INSTRUCCION_POR_ROL: Record<Rol, string> = {
  defensor:
    "El usuario analiza este caso como DEFENSOR. Las preguntas deben ayudar a anticipar la imputación, evaluar admisibilidad de la prueba en contra, identificar causales de exclusión / atenuación / nulidad, y planear estrategia probatoria propia. Foco en debilidades de la acusación.",
  querellante:
    "El usuario analiza este caso como QUERELLANTE (fiscal o particular damnificado). Las preguntas deben ayudar a configurar tipos penales y agravantes, asegurar prueba propia (vínculo, daño, autoría), y anticipar la defensa. Foco en construir un caso sólido.",
  ambos:
    "El usuario analiza este caso como AMBOS (defensor y querellante en paralelo). Las preguntas deben servir para construir las dos estrategias. NO dupliques: una misma pregunta que sirva a las dos perspectivas se hace UNA SOLA VEZ. Cuando una pregunta es asimétrica (solo sirve a un lado), priorizá las que tengan mayor impacto en la decisión de estrategia.",
};
...
REGLAS:
- Generá entre 4 y 8 preguntas máximo, las más relevantes para definir la estrategia desde el rol indicado.
