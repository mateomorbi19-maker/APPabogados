...
export const LEXIE_MANUAL_APP = [
  ...
  "LAS SECCIONES DEL MENÚ, en orden: " +
    ...
    "**Mis casos** — el listado de causas; al abrir una está la ficha del expediente (carátula, número, organismo, juzgado, fiscalía, delitos), las personas de la causa (con su DNI y su contacto), los escritos redactados para la causa, los reportes que se le mandaron al cliente, el análisis original y el timeline de movimientos. " +
  ...
  "CÓMO SE HACEN LAS COSAS MÁS PEDIDAS: " +
    ...
    "**Cargar un imputado, una víctima, un querellante o un testigo** → Mis casos → la causa → bloque «Partes», botón Agregar persona; ahí se elige el rol, se carga el DNI, el correo y el teléfono, y se marca si es el cliente. " +
    "**Mandarle novedades al cliente sobre su causa** → Mis casos → la causa → bloque «Reportes al cliente» → «Nuevo reporte»: se elige a quién, la plantilla (la app sugiere una), y se contesta un formulario corto con lo que sólo sabe el abogado. La app arma el borrador con los datos de la causa, el abogado lo lee y lo corrige, y recién ahí lo manda: «Enviar por correo» sale desde su Gmail, y «Enviar por WhatsApp» le muestra el número completo del cliente y le abre ese chat con el mensaje ya escrito para que toque enviar. Los huecos [FALTA: …] bloquean el envío hasta que se completen. " +
    "**Generar un escrito judicial para una causa** → Mis casos → la causa → bloque «Escritos» → «Generar escrito». Paso 1: elegir el modelo entre los 136 del estudio (pestaña «Del estudio», con buscador y categorías) o los propios (pestaña «Míos»). Paso 2: revisar los datos del expediente que van al encabezado —los que falten quedan marcados como [COMPLETAR] en el texto—, completar los datos profesionales (firma, matrícula, domicilios; se guardan para siempre) y escribir instrucciones. El redactor tarda entre 30 y 90 segundos, verifica artículos y busca precedentes. El escrito se abre para corregirlo, se baja en PDF y, cuando ya se presentó en el portal judicial, se marca «Presentado»: eso guarda el PDF definitivo en el timeline de la causa. " +
