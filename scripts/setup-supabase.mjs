...
// El schema (DDL) se aplica una vez con `supabase-schema.sql` en el SQL Editor.
//
// Uso:
...
if (errU) {
  ...
  console.error('¿Ya corriste supabase-schema.sql en el SQL Editor de Supabase?');
  process.exit(1);
}
...
if (faltantes.length) {
  console.error('FALTAN usuarios:', faltantes, '→ corré supabase-schema.sql en el SQL Editor.');
  process.exit(1);
}
